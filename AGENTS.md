...
- IWINV의 모든 구독 메일함 계정은 dev 또는 prod의 MailboxPool에 상태와 함께
  등재합니다(DB 풀 = 번호 장부의 단일 진실원천). 쓰지 않는 번호는 `RETIRED`로
  결번 처리하며, 결번 행의 password는 더미값(`retired-not-in-use`)을 사용합니다.
- 번호 대역(2026-08-25 확정): prod 21~50(21~34 결번, 35~50 가용 풀),
  dev 1·2·51~100(1·2·51~64 결번, 65~83 사용/결번 혼재, 84~100 가용 풀).
  `newdok51~64`는 과거 구독이 걸려 있는 오염 구간이므로 절대 재사용하지 않습니다.
- 풀 확장 시 기존 번호를 재사용하지 않고 `newdok101`부터 새 대역을 사용합니다.
